...
from zigent.llm.agent_llms import LLM_GMINI
...
load_dotenv()
# 从环境变量中读取api_key
api_key = os.getenv('ONE_API_KEY')  # 使用 os.getenv 函数从环境变量中读取名为 'ZISHU_API_KEY1' 的 API 密钥。
base_url = "http://54.173.225.8:3128/v1/"  # 定义一个字符串变量 base_url，存储了智谱 AI 开放平台的 API 基础 URL。
chat_model = "gemini-2.0-flash-thinking-exp-01-21"
...
chat_model = "models/gemini-2.0-flash-thinking-exp"
GOOGLE_API_KEY = api_key  # add your GOOGLE API key here
os.environ["GOOGLE_API_KEY"] = GOOGLE_API_KEY
llm = LLM_GMINI(model_name=chat_model)
"""
import os
...
class WriteDirectoryAction(BaseAction):
    ...
    def __call__(self, **kwargs):
        ...
        result=directory_data[7:-4]
        print("result:",result)
        try:
            directory_data = json.loads(result)
        except:
            directory_data = {"title": topic, "directory": []}
        print("directory_data_json",directory_data)   
        return {
            "topic": topic,
            "language": language,
