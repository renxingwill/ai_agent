...
documents = SimpleDirectoryReader(input_files=['./wow-agent/docs/网红.txt']).load_data() # 使用 SimpleDirectoryReader 读取指定路径下的文本文件，并将读取到的数据加载为 Document 对象列表
...
query_engine_tools = [
    QueryEngineTool(
        ...
        metadata=ToolMetadata(
            ...
            description=(
                "优先使用此工具回答网红、自媒体、营销、销售、品牌推广等营销类相关问题，"                
            ),
        ),
    ),
...
search_tool = FunctionTool.from_defaults(
    fn=zhipu_web_search_tool,
    name="Web_Search",
    description=(
        "当本地知识库无法提供足够信息时，"
        "使用此工具通过互联网搜索获取最新实时信息。"
        "注意：优先使用本地信息，网络信息作补充"
    ),
)
agent = ReActAgent.from_tools(
    tools=query_engine_tools + [search_tool],
    llm=llm,
    verbose=True,
    # max_iterations=3,  # 适当减少迭代次数
    react_mode="plan_and_execute",
    
    # 新增系统提示强化逻辑
    system_prompt=(
        "你是一个严谨的顾问，必须遵守以下工作流程：\n"
        "1. 优先使用 Local_Knowledge_Base 工具\n"
        "2. 如果该工具响应中包含'无法提供'或'建议查询'或'cannot provide'等关键词\n"
        "3. 必须立即使用 Web_Search 工具获取最新信息\n"
  
    )
)
print(agent.chat("商标注册规费最新标准是多少？"))
   
if __name__ == "__main__":
    

    FLAG_CONTINUE = True  # 初始化一个标志变量，用于控制是否继续生成
    while FLAG_CONTINUE:  # 当 FLAG_CONTINUE 为 True 时循环执行
        input_text = input("What tutorial would you like to create?\n")  # 提示用户输入想要创建的教程主题
        response1 = agent.chat(input_text) # 使用 ReAct 代理对问题进行聊天
        print(response1) # 打印代理的响应        
        if input("\nDo you want to create another tutorial? (y/n): ").lower() != "y":  # 询问用户是否要创建另一个教程
            FLAG_CONTINUE = False  # 如果用户输入不是 'y'，则将 FLAG_CONTINUE 设置为 False，结束循环
