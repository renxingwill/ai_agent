...
base_url = "http://49.0.256.98:3002/v1"
...
class WriteDirectoryAction(BaseAction):
    ...
    def __call__(self, **kwargs):
        ...
        def extract_json(text):
            ...
            if end_index == -1:
                raise ValueError("缺失 </think> 标签")
            
            # 截取标签后的内容并去除空白
            json_str = text[end_index + len(end_tag):].strip()
            return json_str

        # 示例用法
        directory_data = extract_json(directory_data)
        print("resultd:",directory_data)
        directory_data = directory_data[7:-4]
        print("res1: ", directory_data)
